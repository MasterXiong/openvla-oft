...
def get_hn_lora_model(model, config_or_dict):
    """Create HN-LoRA model - drop-in replacement for get_peft_model"""
    if isinstance(config_or_dict, dict):
        config = HNLoRAConfig(**config_or_dict)
    else:
        config = config_or_dict
    
    # Apply HN-LoRA and return the modified base model
    return apply_hn_lora_to_base_model(model, config)